...
## NYRA Core Features & Product Experience

### 🚀 High-Value AI Features
* **RAG Confidence / Answer Quality Indicator**: Visual indicator under AI responses showing grounding confidence (e.g., "Answer grounded in 4 sources" or percentage). Use colors (🟢 High, 🟡 Moderate, 🔴 Low).
* **"Ask About This Source"**: Actionable citation expansion that allows starting a new message context based on specific source pages (e.g., "Explain the section from Machine Learning Notes.pdf on page 14").
* **Document Collections & Chat Folders**: Ability to organize documents into semantic collections (e.g., "Semester 1", "Projects") and restrict RAG searches to specific collections or documents via a **Chat Scope Selector**.
* **Document Preview & Citation Highlighting**: Clicking a citation opens a premium PDF-style viewer and highlights the exact retrieved passage used for the response.

### 🤖 Smart Student Assistant Modes
* **Quick AI Actions**: Post-answer prompts like "Explain simpler", "Summarize", "Make notes", "Give examples", "Ask follow-up", "Create quiz".
* **Study Mode**: Specialized modes to extract key concepts, definitions, formulas, flashcards, and MCQs from selected documents.
* **"Teach Me" Mode**: Scaffolded learning flow (Concept -> Simple explanation -> Example -> Analogy -> Check your understanding).
* **Exam Mode**: Interactive test generation based on uploaded documents with customizable difficulty, time limits, and post-exam feedback on strong/weak areas.
* **Research Workspace**: Temporary research boards allowing users to collect sources, evidence, and analyze answers for complex topics.
* **Compare Documents**: Retrieve and synthesize comparative tables across multiple uploaded documents.

### 📊 Knowledge & RAG Visualization
* **Knowledge Analytics Dashboard**: High-level overview of documents, pages, chunk counts, and indexing status.
* **Document Health**: Visual status checklist per document (Uploaded, Extracted, Chunked, Embedded, Indexed).
* **RAG Pipeline & Retrieval Visualization**: Optional animated panel showing data flow (Document -> Chunking -> Embedding -> Retrieval -> LLM). During retrieval, animate the search process ("Searching 24 documents -> 127 chunks evaluated").
* **Neural/Knowledge Graph**: Interactive node-based visualization of the RAG relationship (Retrieval, Docs, LLM, Answer).

### ✨ Premium UX & Navigation
* **Command Palette (Ctrl + K) & Global Search**: Universal search across chats, documents, sources, and system actions.
* **Keyboard Shortcuts**: Native app feel with shortcuts for new chat, upload, search, etc.
* **Pinned Chats & Folders**: Organization tools for long-term project and exam tracking.

### 🎨 Visual & Aesthetic Flourishes
* **Living AI Background**: Background subtly shifts based on AI state (Idle: slow aurora, Thinking: violet pulse, Retrieving: cyan particles, Generating: soft flowing light, Complete: brief emerald pulse).
* **AI Avatar Evolution**: The NYRA logo visually morphs or pulses depending on the processing state (Idle, Thinking, Retrieving, Generating, Success, Error).

### 🛡️ Trust, Explainability & Developer Tools (ML/RAG Project Viva)
* **"Why this answer?" & "Not Found in Knowledge Base"**: Explainable AI components showing retrieved passages and generation context. Graceful fallbacks when information isn't available.
* **User Feedback Mechanism**: Thumbs up/down with detailed categorizations for model evaluation.
* **RAG Evaluation Dashboard & Retrieval Debugger**: Developer-oriented views showing Faithfulness, Answer relevance, Context relevance, retrieval speeds, and Top K similarity scores.
* **RAG Pipeline Playground**: Interactive playground to test chunk sizes, overlap, Top K, and similarity thresholds.

### 🏆 Recommended Implementation Hierarchy
* **V1 (Essential)**: Chat, RAG sources, Document upload/management, Chat history, Auth, Settings, Dark/light mode, Responsive UI, Search, Source expansion.
* **V2 (Premium Product)**: Document collections, Preview, Citation highlighting, Ask About This Source, Quick AI actions, Study Mode, Compare Documents, Command Palette, Knowledge analytics.
* **V3 (ML Showcase)**: RAG pipeline visualization, Retrieval visualization/debugger, Evaluation dashboard, Knowledge graph, Exam mode, Research workspace.
