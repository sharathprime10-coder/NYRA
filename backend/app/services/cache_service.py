...
def get_redis_client():
    ...
    if _redis_client is None:
        try:
            ...
            print(f"Warning: Could not connect to Redis, falling back to memory cache. Error: {e}")
            _redis_client = "memory"
    return _redis_client

def _generate_cache_key(query: str, user_id: int, document_id: Optional[str]) -> str:
...
def get_cached_response(query: str, user_id: int, document_id: Optional[str]) -> Optional[Dict[str, Any]]:
...
def set_cached_response(query: str, user_id: int, document_id: Optional[str], response_data: Dict[str, Any], ttl_seconds: int = 3600):
