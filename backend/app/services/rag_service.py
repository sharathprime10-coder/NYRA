...
def process_and_store_document(file_path: str, document_id: str, user_id: int):
    ...
    if ext in [".mp3", ".wav", ".m4a"]:
        try:
            ...
            doc = LangchainDocument(page_content=transcript, metadata={"source": file_path})
...
def query_knowledge_base(query: str, filters: dict = None):
    ...
    try:
        ...
        dense_retriever = vector_store.as_retriever(search_type="similarity", search_kwargs={"k": 10, "filter": filters})
        ...
        # Fallback completely
        dense_results = vector_store.similarity_search_with_score(query, k=4, filter=filters)
        final_docs = [doc for doc, score in dense_results]
        top_score = 1.0 if not dense_results else (1.0 - dense_results[0][1]) # invert distance
    ...
    if not final_docs or confidence == "Low":
        try:
            shared_results = shared_vector_store.similarity_search_with_score(query, k=2)
            if shared_results:
                shared_min_dist = min([score for doc, score in shared_results])
                if shared_min_dist < 0.6:  # Only fallback if shared FAQ has a good match
    ...
    return {
        ...
        "sources": [
            {
                "document_id": doc.metadata.get("document_id") if not is_shared else "shared_faq",
                "source": doc.metadata.get("source") if not is_shared else "Shared NYRA Knowledge Base",
                "page": doc.metadata.get("page"),
                "content": doc.page_content
            } for doc in final_docs
        ],
        "confidence": confidence,
        "min_distance": 1.0 - top_score  # Mock min_distance so the frontend doesn't break
    }
