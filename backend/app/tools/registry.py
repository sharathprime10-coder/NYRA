...
def rag_tool(query: str, config: RunnableConfig) -> dict:
    ...
    if not user_id:
        return {"error": "GUARDRAIL BLOCK: user_id is required to perform a knowledge base search."}
    ...
    if document_id and user_id:
        ...
        try:
            ...
            if not doc:
                return {"error": "GUARDRAIL BLOCK: You do not have permission to access this document_id or it does not exist."}
        finally:
            db.close()
    ...
    clean_filters = {}
    if "$and" in filters:
        valid_conditions = [cond for cond in filters["$and"] if list(cond.values())[0] is not None]
    ...
    try:
        ...
        try:
            ...
            for source in res["sources"]:
                ...
                resolved_sources.append({"filename": filename, "page": source.get("page"), "document_id": doc_id})
        finally:
            db.close()
