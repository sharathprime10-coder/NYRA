...
def opt_in_knowledge(req: OptInRequest, db: Session = Depends(get_db)):
    ...
    staging = SharedKnowledgeStaging(
        user_query=req.user_query, ai_response=req.ai_response, status="pending"
    )
    db.add(staging)
    db.commit()
...
def get_pending_pairs(db: Session = Depends(get_db), _: bool = Depends(verify_admin)):
    ...
    records = (
        db.query(SharedKnowledgeStaging)
        .filter(SharedKnowledgeStaging.status == "pending")
        .all()
    )
    return [
        StagingResponse(
...
def approve_pair(
    staging_id: str, db: Session = Depends(get_db), _: bool = Depends(verify_admin)
):
    """Admin approves a pair. Adds it to ChromaDB."""
    record = (
        db.query(SharedKnowledgeStaging)
        .filter(SharedKnowledgeStaging.id == staging_id)
        .first()
    )
    if not record:
    ...
    doc = LangchainDocument(
        ...
        metadata={
            "source": "Shared NYRA Knowledge Base",
            "document_id": "shared_faq",
            "type": "qa_pair",
        },
    )
