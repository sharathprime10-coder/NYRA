...
async def upload_document(
    ...
    try:
        ...
        if ext not in allowed_extensions:
            raise HTTPException(
                status_code=400,
                detail=f"File type {ext} not allowed for security reasons.",
            )

        # 2. Create DB entry with sanitized name
        initial_status = "transcribing" if ext in audio_extensions else "processing"
        db_doc = Document(
            filename=safe_original_name, status=initial_status, user_id=current_user.id
        )
        db.add(db_doc)
        db.commit()
        db.refresh(db_doc)
        ...
        background_tasks.add_task(
            process_doc_task, file_path, str(db_doc.id), str(current_user.id)
        )
