...
def get_auth_token(client, db_session, email="docuser@example.com"):
    ...
    if not user:
        user = User(email=email, hashed_password=get_password_hash("password123"), name="Doc User")
        db_session.add(user)
        db_session.commit()
    
    login_response = client.post("/api/auth/login", data={"username": email, "password": "password123"})
...
def test_list_documents_ownership(client, db_session):
    ...
    # Create documents
    doc1 = Document(filename="doc1.pdf", content_type="application/pdf", file_path="/fake/doc1.pdf", user_id=user1.id, status="ready")
    doc2 = Document(filename="doc2.pdf", content_type="application/pdf", file_path="/fake/doc2.pdf", user_id=user2.id, status="ready")
    db_session.add_all([doc1, doc2])
    db_session.commit()
    
    # User 1 should only see doc1
    response1 = client.get("/api/documents/", headers={"Authorization": f"Bearer {token1}"})
    ...
    response2 = client.get("/api/documents/", headers={"Authorization": f"Bearer {token2}"})
